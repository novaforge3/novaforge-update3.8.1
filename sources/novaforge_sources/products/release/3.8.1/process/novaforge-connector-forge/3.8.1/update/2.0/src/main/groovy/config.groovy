...
new File(portalVaadinTheme).eachFile() { currentTheme ->
  ...
  if (currentTheme.isDirectory()) {
    ...
    def faviconLogo = beaverEngine.getResource("main","novaforge-connector-forge","logoFavIcon")
    ...
    def logoHeader = beaverEngine.getResource("main","novaforge-connector-forge","logoHeader")

    if (StringUtils.isNotBlank(logoHeader)) {
      final Path logoHeaderPath = Paths.get(logoHeader);
      if (Files.exists(logoHeaderPath)) {
        beaverEngine.copyToFile(logoHeader, currentThemePath + "/img/logos/novaforge.png")
      }
    }

    //copy logoFooter to vaadin theme
    def logoFooter = beaverEngine.getResource("main","novaforge-connector-forge","logoFooter")

    if (StringUtils.isNotBlank(logoFooter)) {
      final Path logoFooterPath = Paths.get(logoFooter);
      if (Files.exists(logoFooterPath)) {
        beaverEngine.copyToFile(logoFooter, currentThemePath + "/img/logos/company.png")
      }
    }
  }
