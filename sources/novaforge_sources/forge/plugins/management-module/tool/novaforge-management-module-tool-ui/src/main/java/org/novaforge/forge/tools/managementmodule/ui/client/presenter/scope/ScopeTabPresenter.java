...
public class ScopeTabPresenter implements TabPresenter {
  ...
  private Logger logger = Logger.getLogger(ScopeTabPresenter.class.getName()); 
   ...
   public void bind() {
      ...
      this.eventBus.addHandler(SelectScopeUnitEvent.TYPE, new SelectScopeUnitEvent.Handler() {
         ...
         public void selectScopeUnit(final SelectScopeUnitEvent pEvent) { 
           setCurrentScopeUnitDTO(pEvent.getScopeUnitDTO());
         }
      });
   }
   ...
   protected void setCurrentScopeUnitDTO(ScopeUnitDTO scopeUnit) 
   { 
      ...
      return Collections2.filter(pScopeLotList,
            new Predicate<ScopeUnitDTO>() {
               ...
               public boolean apply(ScopeUnitDTO pInput) {
                  boolean ret =  filteredOnScopeByScopeUnit (pInput,pScopeName, pType, pVersion);
                  return ret;
               }
            });
   }
   
   private boolean  filteredOnScopeByScopeUnit (ScopeUnitDTO pInput, final String pScopeName, final String pType, final String pVersion){
     boolean ret = false;
     ...
     if (pInput != null) {
       ...
       for (final ScopeUnitDTO childScopeUnit : pInput.getChildrenScopeUnit()){
         if (ret) {
           break;
         }
         ret = filteredOnScopeByScopeUnit (childScopeUnit, pScopeName, pType, pVersion);
       }
     }
