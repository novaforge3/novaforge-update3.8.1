...
public class ProjectPlanManagerImpl implements ProjectPlanManager {
	...
	public ProjectPlan validateProjectPlan(final String pProjectId) throws ManagementModuleException {
		...
		final Map<Lot, Lot> srcLots = new HashMap<Lot, Lot>();
			
		for (final Lot lot : projectPlan.getLots()) {
			if (lot.getSrcLot() != null) {
			  srcLots.put(lot, lot.getSrcLot());
			}
		}
	  /*  scope unit discipline : new scope units */
	  for(Entry<Lot, Lot> entry : srcLots.entrySet()) {
	    Lot lot = entry.getKey();
	    Lot srcLot = entry.getValue();
      ...
      final String oldVersion = EMPTY_STRING + srcLot.getProjectPlan().getVersion().intValue();
      digitsToRemove = oldVersion.length() + 1;
			...
			for (final ScopeUnit su : srcLot.getScopeEntities()) {
		...
		/* Iteration : new lot */
	  /* Iteration task : new scope units */
	  for(Entry<Lot, Lot> entry : srcLots.entrySet()) {
      Lot lot = entry.getKey();
      Lot srcLot = entry.getValue();
      ...
      digitsToRemove = oldVersion.length() + 1;
			...
			for (final Iteration iteration : srcLot.getIterations()) {
			...
			lot.getIterations().addAll(srcLot.getIterations());
		}
