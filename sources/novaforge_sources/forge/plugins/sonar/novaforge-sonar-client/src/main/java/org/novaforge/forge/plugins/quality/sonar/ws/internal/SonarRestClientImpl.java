...
import org.novaforge.forge.plugins.quality.sonar.ws.marshallers.GroupUnmarshaller;
...
import org.novaforge.forge.plugins.quality.sonar.ws.models.Group;
...
public class SonarRestClientImpl implements SonarRestClient {
	...
	public List<Component> getAllComponents(SonarWSContext sonarWSContext) throws SonarRestException {
	  ...
	  List<Component> ret = new ArrayList<Component>();
        ...
        new GetRequest("api/projects/index"));
        
    List<Project> projectsList = ProjectUnmarshaller.parse(response.content());
    ...
    for (final Project project : projectsList)
    {
      ...
      treeWsRequest.setBaseComponentKey(project.getK());              
      ...
      TreeWsResponse treeResponse = wsClient.components().tree(treeWsRequest);    

      ret.add(treeResponse.getBaseComponent());
      ret.addAll(treeResponse.getComponentsList());   
    }
    ...
    for (final Project project : projectsList)
    {
      ...
      List<Group> groupList = filterGroupProjectPermission (sonarWSContext,forgeProjectId, project.getK());
      
      if (!groupList.isEmpty() && (sonarUser.isProjectUser(forgeProjectId) || sonarUser.isSonarAdmin())){
...
//	}
	...
	@Override
	public void addGroupToProjectPermission(SonarWSContext sonarWSContext, String groupName, String projectKey, String permission) throws SonarRestException {
		
		Project project = this.searchProject(sonarWSContext, projectKey);
		...
		if(project != null){
			...
			addGroupWsRequest.setProjectKey(project.getK());
			
			wsClient.permissions().addGroup(addGroupWsRequest);
			
		} else {
			
			throw new SonarRestException("Project " + projectKey + " not found - no permission associated to the group " + groupName);
		}
		
	}

	@Override
	public void removeGroupFromProjectPermission(SonarWSContext sonarWSContext, String groupName, String projectKey, String permission)  throws SonarRestException {
		...
		if(project != null){
			...
			removeGroupWsRequest.setProjectKey(project.getK());
...
//	}
	...
	@Override
	public TimeMachine getTimeMachine(SonarWSContext sonarWSContext, String resourceId, String metrics,
	  ...
	  TimeMachine timeMachine = null;
		...
		List<TimeMachine> timeMachineList = TimeMachineUnmarshaller.parse(response.content());
		if (!timeMachineList.isEmpty()){
		  timeMachine = timeMachineList.get(0);
		}
	 ...
	 * @return
	 */
	private Project searchProject(SonarWSContext sonarWSContext, String projectKey) throws SonarRestException {
		...
		List<Project> projectList = ProjectUnmarshaller.parse(response.content());
  	...
  	if(!projectList.isEmpty()){
    		...
    		if(projectList.size() == 1){
    			...
    			throw new SonarRestException("Project[" + projectKey + " matches more one project");
    		}
		}
		return ret;
	}
	...
	private List<Group> filterGroupProjectPermission (SonarWSContext sonarWSContext, String forgeProjectId, String projectKey ) throws SonarRestException {
	  ...
	  WsResponse response =  wsClient.wsConnector().call(new GetRequest("api/permissions/groups")
        .setParam("permission", "user")
        .setParam("permission", "admin")
        .setParam("permission", "codeviewer")
        .setParam("projectKey", projectKey));
    
   
    Groups groups = GroupUnmarshaller.parse(response.content());
    
    if (groups != null){
       
        List<Group> groupsList = groups.getGroups();
        
        
        for (final Group group : groupsList){
          if (group.getName().startsWith(forgeProjectId)){  
            ret.add(group);
            }
        }
    }
