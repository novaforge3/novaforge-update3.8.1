  ...
  public void setProjects(final List<Project> pProjects) throws UserServiceException
  {
    ...
    for (final Project project : pProjects)
    {
      ...
      if (!ProjectServices.getProjectPresenter().isForgeProject(itemId))
      {
        ...
        try {
        	final UserProfile userProfile = ProjectServices.getUserPresenter().getUserProfile(project.getAuthor());
            ...
            getContainerProperty(itemId, ProjectItemProperty.AUTHOR.getPropertyId()).setValue(NULL);
            getContainerProperty(itemId, ProjectItemProperty.AUTHOR_LOGIN.getPropertyId()).setValue(NULL);
        }
